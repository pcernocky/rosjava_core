...
public class OutgoingMessageQueue<T> {
  ...
  private volatile boolean latchMode;
  private volatile T latchedMessage;
  ...
  public OutgoingMessageQueue(MessageSerializer<T> serializer, ExecutorService executorService) {
    ...
    channelGroup = new DefaultChannelGroup(GlobalEventExecutor.INSTANCE); // todo
    writer = new Writer();
  ...
  private void writeLatchedMessage(Channel channel) {
    ByteBuf latchedBuffer = channel.alloc().buffer();
    serializer.serialize(latchedMessage, latchedBuffer);
    channel.writeAndFlush(latchedBuffer);
  }
